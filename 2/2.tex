...
\begin{itemize}
    ...
    \item Várható értékre vonatkozó azonosságok:
        \begin{itemize}
            \item Ha $Y=g(X) \Leftarrow E(Y) = E(g(X)) = \int\limits_{-\infty}^{\infty}g(x)\rho(x) \dd x$  
            \item $E\left(\sum\limits_k a_k X_k\right) = \sum\limits_k a_k E(X_k)$
            \item Ha $X_1, X_2, ...$ független változók, akkor $E\left(\prod\limits_k X_k\right) = \prod\limits_k E(X_k)$
        \end{itemize}  
\end{itemize}
